...
par(old.par)
g <- erdos.renyi.game(n=1000, 0.005, type = 'gnp')
...
node_size <- rescale(nchar = degree(g), low = 2, high = 8)
...
plot(degree.distribution(g), xlab = "Degree", ylab = "Proportion", log = 'xy')

par(old.par)
g <- barabasi.game(500, out.dist = c(0.25, 0.5, 0.25), directed = F, zero.appeal = 1)
V(g)$color <- "lightblue"
V(g)[degree(g) > 9]$color <- "red"
node_size <- rescale(nchar = degree(g), low = 2, high = 8)
plot(g, vertex.label = NA, vertex.size = node_size)

g1 <- barabasi.game(10, m=1, directed = F)
g2 <- barabasi.game(25, m=1, directed = F)
g3 <- barabasi.game(50, m=1, directed = F)
g4 <- barabasi.game(100, m=1, directed = F)
par(mfrow = c(2,2), mar = c(4,0,1,0))
...
g_smwrld <- watts.strogatz.game(dim = 1, size = 1283, nei = 2, p = 0.25)
g_prfatt <- barabasi.game(1283, out.dist = c(0.15, 0.6, 0.25), directed = F, zero.appeal = 2)
