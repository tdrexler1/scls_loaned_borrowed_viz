library(statnet)
library(UserNetR)
op <- par(mar = c(5.1, 4.1, 4.1, 2.1), mfrow = c(1,1))
data(Bali)
...
gplot(ndum, gmode = "graph", vertex.cex = 2, vertex.col = rgb(0,0,139, alpha = 80, maxColorValue = 255),
      edge.col = "grey80", edge.lwd = 0.5, main = "Partly transparent")
par(op)

rolelab <- get.vertex.attribute(Bali, "role")
par(mar = c(0,0,0,0), mfrow = c(1,1))
plot(Bali, usearrows = F, vertex.cex = 1.5, label = rolelab, displaylabels = T, vertex.col = "role")
par(op)
...
sidenum <- 3:7
plot(Bali, usearrows = F, vertex.cex = 4, displaylabels = F, vertex.sides = sidenum[rolecat])
...
deg <- degree(Bali, gmode = "graph"); deg
cls <- closeness(Bali, gmode = "graph"); cls
bet <- betweenness(Bali, gmode = "graph"); bet

par(mar = c(0,0,2,1), mfrow = c(1,2))
...
par(mar = c(0,0,0,0), mfrow = c(1,1))
plot(Bali, displaylabels = T, label.cex = 0.8, pad = 0.4, label.col = "darkblue")

rolelab <- get.vertex.attribute(Bali, "role")
plot(Bali, usearrows = F, label = rolelab, displaylabels = T, label.col = "darkblue")
