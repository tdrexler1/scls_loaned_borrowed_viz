...
class(net1)
summary(net1)
gplot(net1, vertex.col = 2, displaylabels = T)
...
netval1 <- rbind(c(0,2,3,0,0),
                 ...
                 c(0,0,0,0,0),
                 c(0,0,2,0,0))
netval1 <- network(netval1, matrix.type = "adjacency", ignore.eval = F, names.eval = "like")
...
net.df
setwd("C:\\Users\\tdrex\\Documents\\UW Data Science\\DS 745")
write.csv(net.df, file = "MyData.csv", row.names = F)
...
d.val[d.val < 3] <- 0
d.filt <- as.network(d.val, directed = F, matrix.type = "a", ignore.eval = F, names.eval = "collab")
summary(d.filt, print.adj = F)
gden(d.filt)

op <- par(mar = rep(0,4))
gplot(d.filt, gmode = "graph", displaylabels = T, vertex.col = "lightblue", vertex.cex = 1.3, label.cex = 0.4,
      label.pos = 5, displayisolates = F)
par(op)

op <- par(mar = rep(0,4))
d.val <- as.sociomatrix(d, attrname = "collab")
gplot(d.val, gmode = "graph", thresh = 2, vertex.col = "lightblue", vertex.cex = 1.3, label.cex = 0.4,
      label.pos = 5, displayisolates = F)
par(op)
